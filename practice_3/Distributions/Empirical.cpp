...
Empirical::Empirical(Primary &distiburion, int size)
{
    dataSize = size;
    double *tempData = new double[dataSize];
    for (int i = 0; i < dataSize; i++)
    {
        tempData[i] = distiburion.random_value();
    }
    data = tempData;
}

Empirical::Empirical(Mixture &mixture, int size)
{
    dataSize = size;
    double *tempData = new double[dataSize];
    for (int i = 0; i < dataSize; i++)
    {
        tempData[i] = mixture.random_value();
    }
    data = tempData;
}
